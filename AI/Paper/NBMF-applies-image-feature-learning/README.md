...
## 课题简介
​	非负二值矩阵分解（NBMF）适用于需要离散化或稀疏表示的场景，如模式识别、生物信息学、推荐系统等。该论文提出了D-Wave量子退火器在非负二值矩阵分解问题中的应用研究，将其应用于面部图像特征学习，并与两种经典方法（qbsolv 和 Gurobi）进行性能对比。实验结果显示，在特定的性能指标下，D-Wave在部分情况下优于经典方法，但整体性能与经典工具各有优劣。
​	本课题旨在对原论文实验进行复现，将测试使用量子退火计算NBMF的效果，并在可能情况下与经典求解器（如模拟退火和Gurobi）进行对比。本项目遵循量子开发实验室的代码规范和项目结构要求。
